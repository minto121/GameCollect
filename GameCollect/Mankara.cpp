...
void Mankara::MoveStone()
{
	...
	if (MyTurn == FALSE && PocketEnter == 1&& P2Pocket[0] == TRUE) {
		...
		for (int i = 0; i < tmp2;) {
			...
			if (i > 12) { //石の数が13より上だったら、
				P2StoneSave[i - 12] += 1;  // 自分のポケットに石を入れる処理
			}
		}
		// 移動が終わったので、石の数を０にする。
		if (P2StoneSave[0] >= 13) {
		...
		for (int i = 0; i < tmp2;) {
			...
			if (i > 11) { //石の数が12より上だったら、
				P2StoneSave[i - 11] += 1;  // 自分のポケットに石を入れる処理
			}
		}
		// 移動が終わったので、石の数を０にする。
		if (P2StoneSave[1] >= 13) {
		...
		for (int i = 0; i < tmp2;) {
			...
			if (i > 10) { //石の数が11より上だったら、
				P2StoneSave[i - 10] += 1;  // 自分のポケットに石を入れる処理
			}
		}
		// 移動が終わったので、石の数を０にする。
		// 移動が終わったので、石の数を０にする。
		...
		for (int i = 0; i < tmp2;) {
			...
			if (i > 9) { //石の数が10より上だったら、
				P2StoneSave[i - 9] += 1;  // 自分のポケットに石を入れる処理
			}
		}
		// 移動が終わったので、石の数を０にする。
		if (P2StoneSave[3] >= 13) {
		...
		for (int i = 0; i < tmp2;) {
			...
			if (i > 8) { //石の数が9より上だったら、
				P2StoneSave[i - 8] += 1;  // 自分のポケットに石を入れる処理
			}
		}
		// 移動が終わったので、石の数を０にする。
		if (P2StoneSave[4] >= 13) {
